...
@pytest.mark.parametrize("command", [
    "subfleet codex -m gpt-5.6-sol -C . -p p.md -o o.md",
    "cd /x && subfleet claude -a me@x -C . -p p.md -o o.md",
    "codex exec --sandbox read-only 'hi'",
    "CODEX_HOME=~/.codex-3 codex exec 'reply ok'",
    "codex-run -H ~/.codex-3 -m gpt-5.6-sol -C . -p p.md -o o.md",
    "nohup ~/bin/codex-run -H ~/.codex-1 -p p.md -o o.md > log 2>&1 &",
    "claude-lane -a me@x -C . -p p.md -o o.md",
    "/Users/example/chief-of-staff/subfleet/bin/subfleet-codex -m x -C . -p p -o o",
    "codex e 'one-liner'",
    "codex review",
    "cd /x && ~/chief-of-staff/subfleet/bin/subfleet-codex -m x -C . -p p -o o",
...
def test_guard_blocks_attached_runner_launches(command):
    ...
    assert decision["reason"].startswith("[sol-attached]")
