...
def send(subject: str, body: str, *, dry_run: bool = False,
    ...
    try:
        ...
        print(f"carpool: notify hook configuration error: {exc}", file=sys.stderr)
        return False
    if not command:
    ...
    try:
        ...
        print(f"carpool: notify hook error: {exc}", file=sys.stderr)
        return False
    if result.returncode != 0:
        print(
            f"carpool: notify hook failed rc={result.returncode}: {result.stderr[:200]}",
            file=sys.stderr,
        )
